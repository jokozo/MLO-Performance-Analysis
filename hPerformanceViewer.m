...
%   Copyright 2023-2024 The MathWorks, Inc.
    ...
    methods
        function obj = hPerformanceViewer(nodes,simulationTime,varargin)
            ...
            obj.pPacketLatencyVector = zeros(numNodes, 1);
            for idx = 1:numBluetoothNodes
                obj.pPacketLatency(idx,1) = obj.BluetoothNodes{idx}.ID;
                obj.pPacketLatencyVector(idx,1) = obj.BluetoothNodes{idx}.ID;
            end
            for idx = 1:numWLANNodes
                obj.pPacketLatency(numBluetoothNodes+idx,1) = obj.WLANNodes(idx).ID;
                obj.pPacketLatencyVector(numBluetoothNodes+idx,1) = obj.WLANNodes(idx).ID;
            end
    ...
    methods (Access=private)
        function calculatePacketLatency(obj,srcNode,eventData)
            ...
            obj.pPacketLatencyVector(nodeIdx, end+1) = notificationData.CurrentTime-notificationData.PacketGenerationTime;
    ...
    methods (Access=public)
        ...
        function percentiles = getpPacketLatencyVector(obj, p)
            
            latencies = obj.pPacketLatencyVector;
            percentiles = zeros(size(latencies, 1), 1);

            for i = 1:size(latencies, 1)
                rowData = latencies(i, 2:end);
                nonZeroRow = rowData(rowData ~= 0);
                if ~isempty(nonZeroRow)
                    percentiles(i) = prctile(nonZeroRow, p);
                else
                    percentiles(i) = NaN;
                end
            end
